...
class LSTM_model():
    ...
    def configure(self, conf: Dict[Any, Any] = None,
        ...
        data = data["ftr_vector"]
        array = []
        for i in range(0,5):
            array.append(np.asarray(data[i*24:(i+1)*24]))
        scaler = MinMaxScaler()
        minmax = scaler.fit(array)
        scaled_data = minmax.transform(array)
    ...
    def feature_vector_creation(self, message_value: Dict[Any, Any]) -> Any:
        ...
        if(len(self.feature_vector_array) != 24*5):
    ...
    def build_train_model(self, model_structure: Dict[str, Any]):
        ...
        with tf.device("CPU:0"):
            ...
            self.nn.add(LSTM(model_structure["num_features"], activation = 'tanh', input_shape = (model_structure["n_of_timesteps"], model_structure["num_features"]), return_sequences=True))
